...
function inputControl($pdo)
{
    ...
    if (count($errors) > 0) {
        ...
        try {
            ...
            $result1 = $stmt1->execute([$reason, $creditorId, $debitorId, $amount, date('Y-m-d'), $cardIdDebitor, 0]);
    $       $result2 = $stmt2->execute([$reason, $creditorId, $debitorId, $amount, date('Y-m-d'), $cardIdCreditor, 1]);
